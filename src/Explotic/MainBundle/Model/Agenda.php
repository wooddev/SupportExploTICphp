...
class Agenda {
    ...
    public function generate($jours = null){
        ...
        for ($s = $week; $s<$week+4 && $s<=52; $s++)
        {
            ...
            for($j=1; $j<=6;$j++)
            {                
                $agendaY1->getAgendasWeek()->last()->addAgendaDay(new \Explotic\MainBundle\Model\AgendaDay());
                ...
                $agendaY1->getAgendasWeek()->last()->getAgendasDay()->last()->addCreneau(new \Explotic\MainBundle\Model\Creneau());
        ...
        if($week>48)
        {
            $agendaY2= new \Explotic\MainBundle\Model\AgendaYear();
            $agendaY2->setVal($year +1);
            for ($s = 1; $s<=$week+4-52; $s++)
            {
                $agendaY2->addAgendaWeek(new \Explotic\MainBundle\Model\AgendaWeek());
                $agendaY2->getAgendasWeek()->last()->setVal($s); // Création d'une semaine d'agenda portant le numéro $s
                for($j=1; $j<=5;$j++)
                {
                    $agendaY2->getAgendasWeek()->last()->addAgendaDay(new \Explotic\MainBundle\Model\AgendaDay());
                    ...
                    $agendaY2->getAgendasWeek()->last()->getAgendasDay()->last()->addCreneau(new \Explotic\MainBundle\Model\Creneau());
