...
class AgendaOLD {
    ...
    public function generate($jours = null){
        ...
        $agendaY1= new \Explotic\AgendaBundle\Model\AgendaYear();
        $agendaY1->setVal((int)$year);
        // Le calendrier s'étale sur 4 semaines
        for ($s = $week; $s<$week+4 && $s<=52; $s++)
        {
            $agendaY1->addAgendaWeek(new \Explotic\AgendaBundle\Model\AgendaWeek());
            $agendaY1->getAgendasWeek()->last()->setVal($s); // Création d'une semaine d'agenda portant le numéro $s
            
            // Sur 6 jours
            for($j=1; $j<=6;$j++)
            {                
                $agendaY1->getAgendasWeek()->last()->addAgendaDay(new \Explotic\AgendaBundle\Model\AgendaDay());
                ...
                $agendaY1->getAgendasWeek()->last()->getAgendasDay()->last()->addCreneau(new \Explotic\AgendaBundle\Model\Creneau());
        ...
        if($week>48)
        {
            $agendaY2= new \Explotic\AgendaBundle\Model\AgendaYear();
            $agendaY2->setVal($year +1);
            for ($s = 1; $s<=$week+4-52; $s++)
            {
                $agendaY2->addAgendaWeek(new \Explotic\AgendaBundle\Model\AgendaWeek());
                $agendaY2->getAgendasWeek()->last()->setVal($s); // Création d'une semaine d'agenda portant le numéro $s
                for($j=1; $j<=5;$j++)
                {
                    $agendaY2->getAgendasWeek()->last()->addAgendaDay(new \Explotic\AgendaBundle\Model\AgendaDay());
                    ...
                    $agendaY2->getAgendasWeek()->last()->getAgendasDay()->last()->addCreneau(new \Explotic\AgendaBundle\Model\Creneau());
